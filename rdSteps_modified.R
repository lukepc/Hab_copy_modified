## THIS CODE WAS TAKEN FROM https://raw.githubusercontent.com/basille/hab/master/R/rdSteps.r
    ...
    if (!is.null(mask)) {
      ...
      while (anyNA(pover)) {
        ...
        ## Not simultaneously
          ...
          #print('slrd'); #print(slrd[!complete.cases(pover)])
          #print('rhord'); #print(rhord[!complete.cases(pover)])
          slrd[!complete.cases(pover)] <- sample(dis$dist, sum(!complete.cases(pover)),
                                                 replace = TRUE)
          #print('slrd'); #print(slrd)
          ## Sample turning angles (only those with NAs)
          rhord[!complete.cases(pover)] <- sample(dis$rel.angle,
                                                  sum(!complete.cases(pover)), replace = TRUE)
        }
        ...
        pts <- SpatialPoints(data.frame(x = st$x + cos(alphard) * slrd, 
                                        y = st$y + sin(alphard) * slrd),
                             proj4string = CRS(proj4string(mask)))
